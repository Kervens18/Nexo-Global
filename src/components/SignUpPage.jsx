  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center px-4" style={{
      ...
      <div className="w-full max-w-md bg-black/40 rounded-2xl p-8 shadow-xl flex flex-col items-center">
        ...
        <p className="text-gray-200 mb-8 text-center">Start your free e-commerce store today.</p>
        <form className="w-full flex flex-col gap-4">
          ...
          <div className="relative">
            <input type="password" placeholder="Confirm Password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full py-3 pl-10 pr-4 rounded-lg bg-white/10 border border-white/20 text-white placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-orange-400" />
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-300">
              <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><path d="M12 16v-4" /><circle cx="12" cy="8" r="1" /></svg>
            </span>
          </div>
          <button type="submit" className="w-full py-3 mt-2 rounded-lg font-semibold text-lg bg-gradient-to-r from-orange-400 via-pink-600 to-indigo-700 text-white shadow-lg hover:opacity-90 transition">Sign Up</button>
        </form>
        ...
        <div className="flex flex-col gap-3 w-full">
          ...
          <button className="flex items-center gap-3 w-full py-2 px-4 rounded-lg bg-white/10 border border-white/20 text-white hover:bg-white/20 transition">
            <FaFacebookF className="text-xl text-blue-500" /> Sign up with Facebook
          </button>
          <button className="flex items-center gap-3 w-full py-2 px-4 rounded-lg bg-white/10 border border-white/20 text-white hover:bg-white/20 transition">
            <FaApple className="text-xl text-gray-200" /> Sign up with Apple
          </button>
        </div>
        <div className="mt-6 text-gray-300 text-sm text-center">
          Already have an account?{' '}
          <button type="button" className="text-yellow-300 hover:underline font-semibold" onClick={onLogInClick}>Log In</button>
        </div>
      </div>
    </div>
  );
